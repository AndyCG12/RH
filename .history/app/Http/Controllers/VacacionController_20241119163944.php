...
class VacacionController extends Controller
{
    ...
    public function index()
    {
        ...
        $vacaciones = $vacaciones->map(function ($vacacion) {
            return [
                'id' => $vacacion->id,
                'title' => $vacacion->empleado->nombre . (' ') . $vacacion->empleado->apellidoP . (' ') . $vacacion->empleado->apellidoM, // Ajusta el nombre del campo según tu modelo
                'start' => $vacacion->fecha_inicio,
                'end' => $vacacion->fecha_fin +1,
