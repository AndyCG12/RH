            ...
            <table id="data-table-buttons" width="100%" class="table table-striped table-bordered align-middle text-nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td >{{ $nomina->razonSocial->nombre }}</td>
                            <td >{{ '$' . number_format($nomina->salario, 2) $nomina->salario }}</td>
                            <td nowrap>
                        ...
                        <select name="razon_social_id" id="razon_social_id" class="form-control">
                            ...
                            @foreach($razones as $razon)
                                <option value="{{ $razon->id }}"
                                    {{ old('razon_social_id', $nomina->razon_social_id ?? '') == $razon->id ? 'selected' : '' }}>
                                    {{ $razon->nombre }}
                                </option>
                            @endforeach
                        </select>
            ...
            <form action="{{ route('nomina.update', $nomina->id) }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-group">
                            ...
                            <select name="razon_social_id" id="razon_social_id" class="form-control">
                                ...
                                @foreach($razones as $razon)
                                    <option value="{{ $razon->id }}"
                                        {{ old('razon_social_id', $nomina->razon_social_id ?? '') == $razon->id ? 'selected' : '' }}> {{ $razon->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    <div class="mb-3">
                        ...
                        <input type="number" name="salario" class="form-control" value="{{ $nomina->salario }}" required>
                    </div>
                </div>
                <div class="modal-footer">
