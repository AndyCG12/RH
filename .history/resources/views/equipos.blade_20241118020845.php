        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-warning text-white">
                    <h4 class="modal-title" id="modalLabelEditarrazonSocial">Editar equipo</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('razones.update', $equipo->id) }}" method="POST"
                        ...
                        <div class="col-5">
                            ...
                            <input type="text" class="form-control" id="direccion" name="direccion"
                                value="{{ $equipo->direccion }}" required>
                        </div>
                        <div class="col-5">
                            <label for="rfc" class="form-label">RFC</label>
                            <input type="text" class="form-control uppercase-input" id="rfc" name="rfc"
                                value="{{ $equipo->rfc }}" required maxlength="13">
                        </div>
                        <button type="submit" class="btn btn-primary"
                            data-name="{{ $equipo->nombre }}">Actualizar</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-danger text-white">
                    <h4 class="modal-title" id="modalLabelEliminarrazonSocial">Eliminar equipo</h4>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <p>¿Estás seguro de que deseas eliminar al equipo <strong>{{ $equipo->nombre }}
                            {{ $equipo->direccion }} {{ $equipo->rfc }}</strong>?</p>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('razones.destroy', $equipo->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
