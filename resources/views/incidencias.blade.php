                ...
                <table id="data-table-buttons" width="100%" class="table table-striped table-bordered align-middle text-nowrap">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td >
                                    @if($incidencia->empleado->nombre)
                                    {{ $incidencia->empleado->nombre }}
                                    @else
                                    <span>Empleado no disponible</span>
                                    @endif
                                </td>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('incidencia.update', $incidencia->id) }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="titulo-{{ $incidencia->id }}" class="form-label">Título</label>
                        <input type="text" class="form-control" id="titulo-{{ $incidencia->id }}" name="titulo" value="{{ $incidencia->titulo }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="descripcion-{{ $incidencia->id }}" class="form-label">Descripción</label>
                        <textarea class="form-control" id="descripcion-{{ $incidencia->id }}" name="descripcion" rows="3" required>{{ $incidencia->descripcion }}</textarea>
                    </div>
                    <div class="mb-3">
                        <label for="empleado_id-{{ $incidencia->id }}" class="form-label">Empleado</label>
                        <select class="form-select" id="empleado_id-{{ $incidencia->id }}" name="empleado_id" required>
                            @foreach($empleados as $empleado)
                                <option value="{{ $empleado->id }}" {{ $incidencia->empleado_id == $empleado->id ? 'selected' : '' }}>
                                    {{ $empleado->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        <label for="estado-{{ $incidencia->id }}" class="form-label">Estado</label>
                        <select class="form-select" id="estado-{{ $incidencia->id }}" name="estado" required>
                            @foreach($estados as $estado)
